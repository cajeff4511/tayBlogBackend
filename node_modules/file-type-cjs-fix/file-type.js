...
const strtok = typeof window !== 'undefined' ? require('./strtok') : require('./strtok-node')
const stream = require('readable-stream')
const { extensions, mimeTypes } = require('./supported')
...
class FileTypeParser {
  ...
  async parse(tokenizer) {
    ...
    if (this.checkString('ID3')) {
      ...
      return exports.fromTokenizer(tokenizer); // Skip ID3 header, recursion
    }
    // Musepack, SV7
    ...
    if (this.check([0x50, 0x4B, 0x3, 0x4])) { // Local file header signature
      try {
        ...
        if (!(error instanceof strtok.EndOfStreamError)) {
          throw error;
        }
      }
  ...
  async readTiffIFD(bigEndian) {
    ...
    for (let ii = 0; ii < numberOfTags; ++ii) {
...
exports.fromStream = async (stream) => {
  const tokenizer = await strtok.fromStream(stream)
  try {
    return exports.fromTokenizer(tokenizer)
  } finally {
    await tokenizer.close()
  }
}

exports.fromTokenizer = async (tokenizer) => {
  try {
...
exports.fromBuffer = async (input) => {
  if (!(input instanceof Uint8Array || input instanceof ArrayBuffer)) {
    throw new TypeError(`Expected the \`input\` argument to be of type 
      \`Uint8Array\` or \`Buffer\` or \`ArrayBuffer\`, got \`${typeof input}\``)
  }
  ...
  // }
  return exports.fromTokenizer(strtok.fromBuffer(buffer))
}

exports.fileTypeStream = async (readableStream, { sampleSize = minimumBytes } = {}) => {
    ...
    readableStream.once('readable', () => {
      (async () => {
        try {
          ...
          try {
            const fileType = await exports.fromBuffer(chunk)
